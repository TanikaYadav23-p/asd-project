        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-6">
          ...
          <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm">
            ...
            <div className="divide-y divide-slate-100">
              {topExporters.map((sup, idx) => (
                <div key={idx} className="flex justify-between items-center py-2.5 text-xs">
                  <div>
                    <h4 className="font-semibold text-slate-700">{sup.exporter}</h4>
                    <p className="text-[10px] text-slate-400 mt-0.5">{sup.shipments} Shipments</p>
                  </div>
                  <div className="text-right">
                    <span className="font-bold text-slate-800 block">₹{(sup.value / 10000000).toFixed(2)} Cr</span>
          ...
          <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm">
            ...
            <div className="divide-y divide-slate-100">
              {topBuyers.map((buyer, idx) => (
                <div key={idx} className="flex justify-between items-center py-2.5 text-xs">
                  <div>
                    <h4 className="font-semibold text-slate-700">{buyer.buyer}</h4>
                    <p className="text-[10px] text-slate-400 mt-0.5">{buyer.shipments} Shipments</p>
                  </div>
                  <div className="text-right">
                    <span className="font-bold text-slate-800 block">₹{buyer.value?.toFixed(2)} Cr</span>
                    <span className="text-[10px] text-green-500 font-medium">▲ {buyer.growth}%</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm">
            ...
            <div className="divide-y divide-slate-100">
              {portWiseExports.map((port, idx) => (
                <div key={idx} className="flex justify-between items-center py-2.5 text-xs">
                  <div>
                    <h4 className="font-semibold text-slate-700">{port.port}</h4>
                    <p className="text-[10px] text-slate-400 mt-0.5">{port.shipments} Shipments</p>
                  </div>
                  <div className="text-right">
                    <span className="font-bold text-slate-800 block">₹{Number(port.value || 0).toFixed(2)} Cr</span>
