...
export default function AuditLogsDashboard() {
  ...
  const metrics = [
    { label: "Total Activities", value: "12,458", change: "▲ 18.6% vs last 30 days", icon: FileSpreadsheet, color: "text-blue-600", bg: "bg-blue-50/80", border: "border-blue-100" },
    { label: "Successful Activities", value: "11,732", change: "▲ 17.8% vs last 30 days", icon: CheckCircle2, color: "text-emerald-600", bg: "bg-emerald-50/80", border: "border-emerald-100" },
    { label: "Failed Activities", value: "726", change: "▼ 6.4% vs last 30 days", icon: XCircle, color: "text-rose-600", bg: "bg-rose-50/80", border: "border-rose-100" },
    { label: "Active Users", value: "48", change: "▲ 14.3% vs last 30 days", icon: Users, color: "text-orange-500", bg: "bg-orange-50/80", border: "border-orange-100" },
    { label: "Login Activities", value: "1,942", change: "▲ 16.2% vs last 30 days", icon: LogIn, color: "text-purple-600", bg: "bg-purple-50/80", border: "border-purple-100" },
  ];
  ...
  return (
    <div className="overflow-y-auto bg-[#f8fafc] text-slate-700 p-6 font-sans text-xs antialiased selection:bg-blue-100">
        ...
        {metrics.map((card, idx) => (
          <div key={idx} className={`bg-white p-4 rounded-xl border ${card.border} shadow-sm flex items-center gap-3.5`}>
            ...
            <div>
              ...
              <p className={`text-[10px] font-semibold mt-0.5 ${card.color.includes('rose') ? 'text-rose-600' : 'text-emerald-600'}`}>
                {card.change}
              </p>
            </div>
          </div>
      ...
      <div className="bg-white p-3.5 rounded-xl border border-slate-200/60 shadow-sm mb-6 grid grid-cols-2 md:grid-cols-6 gap-3.5 items-end">
        {[
          { label: "User", val: "All Users" },
          { label: "Module", val: "All Modules" },
          { label: "Action", val: "All Actions" },
          { label: "Status", val: "All Status" },
        ].map((filter, i) => (
          <div key={i}>
            <label className="text-[10px] font-bold text-slate-900 block mb-1">{filter.label}</label>
            <div className="flex items-center justify-between bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 text-slate-700 font-medium cursor-pointer hover:bg-slate-50">
              <span className="truncate">{filter.val}</span>
        ...
        <div className="col-span-2 md:col-span-1">
          ...
          <div className="relative">
            <input type="text" placeholder="Search by keyword, ID, details..." className="w-full bg-white pl-3 pr-2 py-1.5 rounded-lg border border-slate-200 text-xs focus:outline-none placeholder:text-slate-400 focus:border-blue-500 font-medium" />
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-8 bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
          <div className="p-4 border-b border-slate-100">
            <h2 className="text-sm font-bold text-slate-900">Audit Log Events <span className="text-slate-400 font-medium">(12,458)</span></h2>
          </div>
            ...
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody className="divide-y divide-slate-100 text-slate-600 font-medium">
                {auditEvents.map((row, idx) => (
                  <tr key={idx} className={`hover:bg-slate-50/60 transition-colors cursor-pointer ${idx === 0 ? 'bg-blue-50/20' : ''}`}>
                    ...
                    <td className="p-3">
                      <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold tracking-wide ${row.actionStyle}`}>
                        {row.action}
                      </span>
                    </td>
                    <td className="p-3 text-slate-900 font-semibold">{row.module}</td>
                    <td className="p-3 text-slate-500 max-w-[180px] break-words">{row.details}</td>
                    <td className="p-3 font-mono text-[11px] text-slate-500">{row.ip}</td>
          ...
          <div className="p-3.5 border-t border-slate-100 bg-[#f8fafc]/40 flex items-center justify-between text-[11px] text-slate-500 font-semibold">
            <span>Showing 1 to 10 of 12,458 events</span>
        ...
        <div className="lg:col-span-4 bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 space-y-5">
          ...
          <div className="flex items-center justify-between border-b border-slate-50 pb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-black text-slate-900 leading-tight">Login</h3>
          ...
          <div className="space-y-2.5 text-[11px]">
            {[
              { label: "Date & Time", val: "24 Apr 2025, 09:15 AM (IST)", isBold: true },
              { label: "User", val: "Abhishek B. (Admin)", isBold: true },
              { label: "User ID", val: "USR-0001", isBold: true, isMono: true },
              { label: "Action", val: "Login", isBold: true },
              { label: "Module", val: "Authentication", isBold: true },
              { label: "IP Address", val: "103.21.45.67", isBold: true, isMono: true },
              { label: "Device / Browser", val: "Windows 11 / Chrome 124.0", isBold: true },
              { label: "Location", val: "Mumbai, Maharashtra, India", isBold: true, color: "text-blue-900" },
              { label: "Details", val: "User logged in successfully", isBold: true, color: "text-blue-900" },
              { label: "Session ID", val: "SESS-2025-0424-000567", isBold: true, isMono: true },
              { label: "Status", val: "Success", color: "text-emerald-600", isBold: true },
            ].map((item, index) => (
              <div key={index} className="grid grid-cols-3 gap-2 py-0.5 items-start">
                <span className="text-slate-400 font-semibold">{item.label}</span>
                <span className={`col-span-2 text-right lg:text-left ${item.color || 'text-slate-900'} ${item.isBold ? 'font-bold' : 'font-medium'} ${item.isMono ? 'font-mono' : ''}`}>
                  {item.val}
                </span>
              </div>
            ))}
          ...
          <div>
            ...
            <div className="space-y-2.5 text-[11px]">
              {[
                { label: "MFA Used", val: "Yes" },
                { label: "Login Method", val: "Password" },
                { label: "Login Source", val: "Web Application" },
                { label: "Session Duration", val: "2h 35m 18s" },
              ].map((info, idx) => (
