  ...
  const filteredUploads = useMemo(() => {
    ...
    return uploads.filter(
      (u) =>
        ...
        u.by.toLowerCase().includes(term)
    );
  }, [searchTerm]);
          ...
          <div className="min-w-[560px]">
            ...
            <div className="divide-y divide-gray-100 max-h-96 overflow-y-auto">
              {filteredUploads.map((u, i) => (
                <div key={u.ref + i} className="grid grid-cols-5 items-center px-1 py-3 gap-2">
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-gray-900 truncate">{u.name}</p>
                    <p className="text-xs text-teal-600 truncate">{u.ref}</p>
                  </div>
                  <span className="text-xs text-gray-500">{u.refId}</span>
                  <div className="text-xs text-gray-500">
                    <p>{u.date}</p>
                    <p>{u.time}</p>
                  </div>
                  <span className="text-xs text-gray-700">{u.by}</span>
                  <span className="text-xs text-gray-500">{u.size}</span>
                </div>
              ))}
